  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;">
      ...
      <el-table-column label="拼团状态" align="center">
        <template slot-scope="{row}">
          <span v-if="row.groupStatus == 0">失败</span>
          <span v-else-if="row.groupStatus == 1">组团中</span>
          <span v-else>成功</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="订单状态" align="center">
        <template slot-scope="{row}">
          <span v-if="row.orderState == 1">待付款</span>
          <span v-else-if="row.orderState == 2">待分享</span>
          <span v-else-if="row.orderState == 3">待发货</span>
          <span v-else-if="row.orderState == 4">待收货</span>
...
<script>
  ...
  import add from './groupOrder_add.vue'
